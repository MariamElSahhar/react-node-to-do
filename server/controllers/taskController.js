	...
	if (profileIndex === -1) {
		...
		if (!("status" in newTask) || !("description" in newTask)) {
			...
			if (data[profileIndex].tasks.length === 0) newTask.id = 0;
				...
				newTask.id =
					data[profileIndex].tasks[
						data[profileIndex].tasks.length - 1
					].id + 1;
...
exports.deleteTask = (req, res) => {
	...
	if (profileIndex === -1) {
		...
		const taskID = req.params.id * 1;
		const taskSearch = data[profileIndex].tasks.findIndex(
			(task) => task.id === taskID
		);
		if (taskSearch === -1) {
			res.status(404).json({
	...
	if (profileIndex === -1) {
		...
		if (taskIndex === -1) {
			...
			if ("description" in req.body)
				data[profileIndex].tasks[taskIndex].description =
